 ...
 * bunun yerine her servis <b>yalnızca kendi dilimine</b> sahiptir. order-service
 * dilimi: {@code CUST_ORD}.
 *
 ...
 * ({@code Order.status}) bir saga durum makinesidir ve {@code OrderStatus} enum'u
 * ile modellenir; bu tabloya FK vermez. Tablo şu an legacy/ETL hizası ve referans
 * veri altyapısının her serviste tekdüze olması için tutulur.
 */
@Getter
@Setter
@NoArgsConstructor
...
public class GeneralStatus extends BaseEntity {
    ...
    /** Stabil iş kodu (legacy {@code SHRT_CODE}), ör. {@code FINISHED}. */
    @Column(name = "short_code", nullable = false, length = 50)
    private String shortCode;

    /** Durumun ait olduğu entity'nin kısa kodu (legacy {@code ENT_CODE_NAME}), ör. {@code CUST_ORD}. */
