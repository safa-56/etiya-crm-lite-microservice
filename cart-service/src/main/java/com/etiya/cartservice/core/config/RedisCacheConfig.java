    ...
    public RedisCacheManager cacheManager(RedisConnectionFactory connectionFactory) {
        return RedisCacheManager.builder(connectionFactory)
                ...
                .withInitialCacheConfigurations(Map.of(
                        // Sepet içeriği sık değiştiğinden tekil sepet cache'i kısa TTL alır.
                        CacheNames.CARTS, baseConfig(Duration.ofMinutes(5)),
                        CacheNames.CART_LIST, baseConfig(Duration.ofMinutes(2))
                ))
                .build();
    }
}
