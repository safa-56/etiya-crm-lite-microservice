...
-- account-service (accountdb), product-service (productdb), cart-service (cartdb),
-- order-service (orderdb) ve search-service (searchdb) kendi veritabanlarına ihtiyaç
-- duyduğundan burada oluşturulur. Her servis kendi şemasına/Debezium slot'una sahiptir
-- (search-service'in outbox'ı/connector'ı yoktur; yalnızca tüketicidir).
--
...
-- veritabanlarını elle oluşturmak için: CREATE DATABASE accountdb; / CREATE DATABASE productdb; / CREATE DATABASE cartdb; / CREATE DATABASE orderdb; / CREATE DATABASE searchdb;
-- =============================================================================
