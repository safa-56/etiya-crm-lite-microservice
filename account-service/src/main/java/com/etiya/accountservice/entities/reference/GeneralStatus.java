package com.etiya.accountservice.entities.reference;

import com.etiya.accountservice.entities.BaseEntity;
 ...
 * bunun yerine her servis <b>yalnızca kendi dilimine</b> sahiptir. account-service
 * dilimi: {@code CUST_ACCT}, {@code CUST_ACCT_PROD_INVL}.
 *
 ...
 *
 * <p><b>Mevcut kullanım notu:</b> Bu servisin çalışan durum alanı
 * ({@code BillingAccount.accountStatus}) bir saga durum makinesidir ve
 * {@code AccountStatus} enum'u ile modellenir; bu tabloya FK vermez. Tablo şu an
 * legacy/ETL hizası ve referans veri altyapısının her serviste tekdüze olması
 * için tutulur.
 */
@Getter
@Setter
...
public class GeneralStatus extends BaseEntity {
    ...
    /** Durumun ait olduğu entity'nin kısa kodu (legacy {@code ENT_CODE_NAME}), ör. {@code CUST_ACCT}. */
    @Column(name = "entity_code_name", nullable = false, length = 100)
    private String entityCodeName;

    /** Durumun ait olduğu entity'nin adı (legacy {@code ENT_NAME}), ör. {@code CUSTOMER_ACCOUNT}. */
